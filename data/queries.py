...
# Retrieves five releases associated with given artist id and no other artists
...
# Get five releases of specific type_id
...
# Delete releasee type and delete or replace associated releases
...
def exists_release_type(name):
    ...
    if db.isOpen():
        try:
            query = QSqlQuery()
            query.prepare('SELECT * from release_type WHERE name = :name')
            query.bindValue(':name', name.lower())
            if not query.exec():
...
def exists_format(name):
    ...
    if db.isOpen():
        try:
            query = QSqlQuery()
            query.prepare('SELECT * from format WHERE name = :name')
            query.bindValue(':name', name.lower())
            if not query.exec():
